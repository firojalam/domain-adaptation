...
#        gx_sym = sparse.csr_matrix('gx', dtype = 'float32')
        ...
        l_x_in1=lasagne.layers.dropout(l_x_in1, p=0.02)
...
#        self.train_fn = theano.function([x_sym, y_sym], loss, updates = updates)
        ...
        updates = lasagne.updates.adadelta(loss - (val * domain_loss), params, learning_rate = 0.1)
        ...
        py_sym = lasagne.layers.get_output(l_x_in1)
        self.test_fn_last_layer = theano.function([x_sym], py_sym)
...
#            print(gy)        
    ...
    def predict_last_layer(self, tx):
        """predict the dev or test instances.
        tx (scipy.sparse.csr_matrix): feature vectors for dev instances.

        returns (numpy.ndarray, #instacnes * #classes): classification probabilities for dev instances.
        """
        return self.test_fn_last_layer(tx)
