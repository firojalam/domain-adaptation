...
#x,y,le,labels=get_data(train_file)
#tx,ty,le,tlabels=get_data(test_file)
...
#m.init_train(init_iter_label = 1200, init_iter_graph = 2000)  # pre-training
...
m = model(args) # initialize the model
m.load_params() # load the model if better result is obtained        
tpy = m.predict(tx)  
accu,P,R,F1,AUC,report=performance_measure(tpy, ty,le,tlabels)
