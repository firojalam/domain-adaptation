...
from ind_model_domain_adaptation_labeled import ind_model as model
import argparse
import cPickle
import optparse
import datetime
...
parser.add_argument('--domain_labeled', help = 'domain labeled file', type = str, default = "")
...
domain_labeled_file=args.domain_labeled
...
domain_labeled_x,domain_labeled_y,domain_labeled_le,domain_labeled_labels,domain_labeled_ids=data_process.getDevData2(devFile,tokenizer,MAX_SEQUENCE_LENGTH,delim)

domain_data=cnn(domain_data_x)
domain_test=cnn(domain_test_x)
domain_labeled_x=cnn(domain_labeled_x)
    
print(len(graph))
print(allx.shape)

m = model(args)                                                 # initialize the model
m.add_data(train_x, train_y, allx_data, graph,domain_data,domain_labeled_x,domain_labeled_y)                                   # add data
