...
class ind_model(base_model):
    ...
    def add_data(self, x, y, allx, graph,domain_data,domain_labeled_x,domain_labeled_y):
        ...
        """
        self.x, self.y, self.allx, self.graph,self.domain_x,self.domain_labeled_x,self.domain_labeled_y = x, y, allx, graph,domain_data,domain_labeled_x,domain_labeled_y
...
#        gx_sym = sparse.csr_matrix('gx', dtype = 'float32')
        ...
        #R,C=self.x.shape
        #l_x_in1=layers.DenseLayer(l_x_in, 512, nonlinearity = lasagne.nonlinearities.rectify)
        #l_x_in1=lasagne.layers.dropout(l_x_in1, p=0.01)
...
#        self.train_fn = theano.function([x_sym, y_sym], loss, updates = updates)
        ...
        updates = lasagne.updates.adadelta(loss - (val * domain_loss), params, learning_rate = 1.0)
...
#        for start_idx in range(0, self.x.shape[0] - self.g_sample_size + 1, self.g_sample_size):
    ...
    def iterate_minibatches_data_labeled(self,inputs, targets, batchsize):
...
#            print 'batch graph context loss', loss
        ...
        for batch in self.iterate_minibatches_data_labeled(self.domain_labeled_x,self.domain_labeled_y, self.batch_size):
            inputs, targets = batch
            source_tr_y=[1] * len(targets)
            y = np.zeros((len(targets), self.nb_classes))
            y[np.arange(len(targets)), source_tr_y] = 1
            source_tr_y=y.astype('int32')            
            loss = self.train_fn(inputs, targets,source_tr_y)            
