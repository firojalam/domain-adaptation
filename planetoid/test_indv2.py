...
parser.add_argument('--batch_size', help = 'batch size for supervised loss', type = int, default = 32)
parser.add_argument('--g_batch_size', help = 'batch size for graph context loss', type = int, default = 32)
parser.add_argument('--g_sample_size', help = 'batch size for label context loss', type = int, default = 32)
...
train_x,train_y,train_le,train_labels,_,_=data_process.getDevDataWithID(trainFile,tokenizer,MAX_SEQUENCE_LENGTH,delim)
delim="\t"
dev_x,dev_y,dev_le,dev_labels,_,dev_ids=data_process.getDevDataWithID(devFile,tokenizer,MAX_SEQUENCE_LENGTH,delim)
test_x,test_y,test_le,test_labels,_,test_ids=data_process.getDevDataWithID(testFile,tokenizer,MAX_SEQUENCE_LENGTH,delim)
delim="\t"
allx,_,_,_,_,_=data_process.getDevDataWithID(allxFile,tokenizer,MAX_SEQUENCE_LENGTH,delim)
...
m.add_data(train_x, train_y, allx_data, graph)                  # add data
...
#while True:
for epoch in range(num_epochs):    
    #m.step_train(max_iter = 10, iter_graph = 10, iter_inst = 10, iter_label = 10) # perform a training step
    #m.step_train(max_iter = 1, iter_graph = 0.1, iter_inst = 1, iter_label = 0)
    m.step_train_minibatch(iter_label = 40)
...
#predictions(test_y,tpy,test_le,test_file,test_ids)
