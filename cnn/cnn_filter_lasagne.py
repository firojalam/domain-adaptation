...
def build_convpool_max(l_in,emb_model,word_index,MAX_NB_WORDS,EMBEDDING_DIM,MAX_SEQUENCE_LENGTH):
    ...
    print (nb_words)
    embedding_layer = lasagne.layers.EmbeddingLayer(l_in, input_size = nb_words, output_size = EMBEDDING_DIM, W=embedding_matrix) #embedding_matrix
    ...
    for filter_len,nb_filter in zip(filter_window_sizes,num_filters):
        conv = Conv1DLayer(embedding_layer,nb_filter, filter_len, stride=1, pad='valid',nonlinearity=lasagne.nonlinearities.rectify)
...
#    emb_model=""
    ...
    train_x,train_y,train_le,train_labels,_,tr_ids=data_process.get_dev_data_with_id(train_file, tokenizer, MAX_SEQUENCE_LENGTH, delim)
    dev_x,dev_y,dev_le,dev_labels,_,dev_ids=data_process.get_dev_data_with_id(dev_file, tokenizer, MAX_SEQUENCE_LENGTH, delim)
    test_x,test_y,test_le,test_labels,_,test_ids=data_process.get_dev_data_with_id(test_file, tokenizer, MAX_SEQUENCE_LENGTH, delim)
    domain_test_x,domain_test_y,domain_test_le,domain_test_labels,_,domain_ids=data_process.get_dev_data_with_id(domain_test_file, tokenizer, MAX_SEQUENCE_LENGTH, delim)
    ...
    convpool = DenseLayer(l_in,num_units=512, W=lasagne.init.GlorotUniform(),nonlinearity=lasagne.nonlinearities.rectify)     
    convpool = lasagne.layers.dropout(convpool, p=.02)
    ...
    for epoch in range(num_epochs):
        ...
        y_prob=[0]
        devoutFile="dev.txt"
        accu,P,R,F1,wAUC,AUC,report=performance.performance_measure_tf(dev_y,dev_pred,y_prob,dev_le,dev_labels,devoutFile)
        wauc=wAUC*100
        auc=AUC*100
        precision=P*100
        recall=R*100
    ...
    accu,P,R,F1,wAUC,AUC,report=performance.performance_measure_tf(test_y,test_pred,y_prob,test_le,test_labels,devoutFile)
    wauc=wAUC*100
    auc=AUC*100
    precision=P*100
    recall=R*100
    ...
    test_pred=np.argmax(test_pred, axis = 1)    
    accu,P,R,F1,wAUC,AUC,report=performance.performance_measure_tf(domain_test_y,test_pred,y_prob,domain_test_le,domain_test_labels,devoutFile)
    wauc=wAUC*100
    auc=AUC*100
    precision=P*100
    recall=R*100
