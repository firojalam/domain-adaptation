...
if __name__ == '__main__':
    ...
    # lambda_val = T.scalar(name='lambda_val')
    # val = T.mean(source_loss) - (domain_loss)
    # print val
    updates1 = lasagne.updates.adadelta(source_loss - (val * domain_loss), source_params,
                                        learning_rate=1.0)  # update blue and green part
    updates2 = lasagne.updates.adadelta(domain_loss, list(set(domain_params) - common), learning_rate=1.0)
    updates3 = lasagne.updates.adadelta(-(val * domain_loss), list(common), learning_rate=1.0)
    ...
    # updates.update(lasagne.updates.nesterov_momentum(domain_loss, list(set(domain_params) - common), learning_rate=0.0002, momentum=0.9))  # update pink part
    ...
    ##compilation
    # train_fn = theano.function([train_l_in.input_var, train_y_var,l_mask.input_var], loss, updates=updates)
    # train_fn = theano.function([train_l_in.input_var,train_y_var,domain_l_in.input_var,domain_y_var], [], updates=updates)

    val_fn = theano.function([l_in.input_var, train_y_var], [test_loss, test_acc])
    domain_test_prediction = lasagne.layers.get_output(domain_network, deterministic=True)
    pred_fn = theano.function([l_in.input_var], test_prediction)
    domain_pred_fn = theano.function([l_in.input_var], domain_test_prediction)
    patience = 30
    best_valid = 0
    best_valid_epoch = 0
    best_weights = None
